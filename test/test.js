...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( sdot.length, 1, 'has expected arity' );
	t.end();
});
...
tape( 'the function calculates the dot product of two one-dimensional ndarrays', function test( t ) {
	...
	xbuf = new Float32Array( [ 4.0, 2.0, -3.0, 5.0, -1.0, 2.0, -5.0, 6.0 ] );
	ybuf = new Float32Array( [ 2.0, 6.0, -1.0, -4.0, 8.0, 8.0, 2.0, -3.0 ] );
	x = vector( xbuf, 8, 1, 0 );
	y = vector( ybuf, 8, 1, 0 );
	v = sdot( [ x, y ] );
	...
	xbuf = new Float32Array( [ 3.0, -4.0, 1.0 ] );
